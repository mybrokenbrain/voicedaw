...
public:
    ...
    /**
     * Detect pitch using the Yin algorithm with amplitude gating and frequency bounds.
     *
     * @param buffer Audio samples (mono)
     * @param threshold Yin algorithm cumulative mean difference threshold (0.1–0.15 typical)
     * @param minAmplitude RMS floor below which to reject as silence (default 0.005)
     * @return Fundamental pitch in Hz (within 60–2000 Hz vocal range), or -1.0f if unvoiced/silent/out-of-range
     */
    float getPitch(const float* buffer, float threshold = 0.15f, float minAmplitude = 0.005f) {
        // Measure RMS to detect silence / very low SNR
        float rms = 0.0f;
        for (int i = 0; i < mHalfBufferSize; i++) {
            rms += buffer[i] * buffer[i];
        }
        rms = std::sqrt(rms / mHalfBufferSize);
        if (rms < minAmplitude) return -1.0f; // Too quiet or no signal
        ...
        if (!foundTau) {
            // Fallback: find global minimum if threshold not met
            float minVal = 1.0f;
            for (int tau = 1; tau < mHalfBufferSize; tau++) {
        ...
        if (tauEstimate > 0) {
            ...
            // Clamp betterTau to avoid very small denominators or absurdly high frequencies
            // For 48kHz, tau < 10 samples → freq > 4.8kHz (unrealistic for voice)
            if (betterTau < 10.0f) betterTau = 10.0f;

            pitchInHertz = static_cast<float>(mSampleRate) / betterTau;

            // Sanity check: vocal range is ~60–2000 Hz (catches octave errors)
            // 60 Hz = ~81 samples at 48kHz, 2000 Hz = ~24 samples
            if (pitchInHertz < 60.0f || pitchInHertz > 2000.0f) {
                return -1.0f; // Out of voice range, reject
            }
        }
